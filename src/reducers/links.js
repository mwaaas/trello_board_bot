...
import { normalizeAndMergeEntitiesBy, normalizeEntitiesById } from '../utils';
...
export default (state = initialState, action) => {
  switch (action.type) {
    ...
    case linkTypes.GET_LINKS_SUCCESS: {
      const linksById = {};
      action.payload.links.forEach((link) => { linksById[link._id] = link; });
      return state
        .merge({ isLoaded: true })
    ...
    case linkTypes.GET_LINK_SUCCESS: {
      const { link, syncHistory } = action.payload;
    ...
    case linkTypes.SYNC_LINK_SUCCESS: {
      ...
      return state.merge({ entities: state.get('entities').set(link._id, fromJS(link)) });
    }
    ...
    case providerIdentityTypes.GET_ALLOWED_PROVIDER_IDENTITIES_SUCCESS: {
      const {
        meta: { containerId, providerIdentityId, formName },
        payload: { providerIdentityIds },
      } = action;
    ...
    case fieldTypes.ADD_FIELD_ASSOCIATION: {
      const { fieldAssociation, entity } = action.payload;
      if (entity !== linkTypes.ENTITY_NAME) {
        return state;
      }
    ...
    case fieldTypes.CHANGE_FIELD_ASSOCIATION_TARGET: {
      const { index, target, entity } = action.payload;
      if (entity !== linkTypes.ENTITY_NAME) {
        return state;
      }
    ...
    case linkTypes.GET_LINKS_SYNC_STATES_SUCCESS: {
      const { linkStates } = action.payload;
    ...
    case fieldTypes.UNMAP_FIELD_ASSOCIATION_ITEM: {
      const {
        fieldAssociationIndex, containerSide, groupIndex, itemIndex, entity,
      } = action.payload;
    ...
    case fieldTypes.REMOVE_FIELD_ASSOCIATION_GROUP: {
      const { fieldAssociationIndex, groupIndex, entity } = action.payload;
    ...
    case linkTypes.LINK_REVIEW_SUCCESS: {
      const { multiSync } = action.payload;
      return state
