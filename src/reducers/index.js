...
import { formUtils, otherSide as getOtherSide } from '../utils';
...
const { CUSTOM_FIELD, PCD_FIELD, PCD_COMMON } = fieldTypes.KINDS;

export const getProviderIdentitiesFromSignUpIntents = (state) => {
  const intentA = fromAuth.getSignupIntentA(state.auth) || '';
  const intentB = fromAuth.getSignupIntentB(state.auth) || '';

  const userId = fromAuth.getUserId(state.auth);
  const userProviderIdentities = fromProviderIdentities.getUserProviderIdentities(
    state.providerIdentities,
    userId,
  );

  const providerIdentityA = userProviderIdentities.find(pi => pi.get('providerName').toLowerCase() === intentA.toLowerCase(), null, Map());
  const providerIdentityB = userProviderIdentities.find(pi => pi.get('providerName').toLowerCase() === intentB.toLowerCase(), null, Map());
  return Map({
    A: providerIdentityA,
    B: providerIdentityB,
  });
...
export const getInitialProviderIdentitiesFromLastLink = (state) => {
  const allLinks = getAllSyncsWithoutMultisync(state);
  if (allLinks.isEmpty()) {
    return allLinks;
  }

  const lastCreatedSync = allLinks.last();
  return Map({
    A: lastCreatedSync.getIn(['A', 'providerIdentity'], Map()),
    B: lastCreatedSync.getIn(['B', 'providerIdentity'], Map()),
  });
};

export const getInitialProviderIdentities = (state) => {
  const idFromQueryParamsA = fromApp.getDefaultParamProviderIdentityId(state.app, 'A');
  const idFromQueryParamsB = fromApp.getDefaultParamProviderIdentityId(state.app, 'B');
  if (idFromQueryParamsA && idFromQueryParamsB) {
    return Map({
      A: getProviderIdentityById(state, idFromQueryParamsA),
      B: getProviderIdentityById(state, idFromQueryParamsB),
    });
  }

  const providerIdentitiesBySync = getInitialProviderIdentitiesFromLastLink(state);
  const providerIdentitiesSignupIntents = getProviderIdentitiesFromSignUpIntents(state);

  if (idFromQueryParamsA) {
    return Map({
      A: getProviderIdentityById(state, idFromQueryParamsA),
      B: providerIdentitiesBySync.get('B') || providerIdentitiesSignupIntents.get('B'),
    });
  }

  if (idFromQueryParamsB) {
    return Map({
      A: providerIdentitiesBySync.get('A') || providerIdentitiesSignupIntents.get('A'),
      B: getProviderIdentityById(state, idFromQueryParamsB),
    });
  }

  if (!providerIdentitiesBySync.isEmpty()) {
    return providerIdentitiesBySync;
  }

  return providerIdentitiesSignupIntents;
};
...
export const getCurrentContainerBySide = (state, { containerSide }) => {
...
export const getWorkspaces = (state, { providerIdentityId }) => fromContainers.getWorkspaces(state.containers, providerIdentityId);

export const getContainers = (state, { containerSide }) => fromContainers.getContainers(state.containers, containerSide);
...
export const getSignupIntentA = state => fromAuth.getSignupIntentA(state.auth);
...
export const getDefaultProviderId = (state, containerSide) => {
  ...
  let providerId = getContainerFieldValue(state, { containerSide }, 'providerId');

  // Remove the other side selected provider from possible options
  const otherProviderId = getOtherSideContainerFieldValue(state, { containerSide }, 'providerId');
...
export const getDefaultProviderIdentityByProviderId = (state, providerId) => {
  ...
  const userId = getUserId(state);
  const activeProviderIdentities = providerIdentityList
    .filter(pi => pi.get('state') === providerIdentityTypes.STATE.ACTIVE)
    .filter(pi => pi.get('users').contains(userId));
...
export const getProviderCapabilitiesMergeableOptions = (state) => {
  const providerA = getProvider(state, { containerSide: 'A' });
  const providerB = getProvider(state, { containerSide: 'B' });

  const optionsA = getProviderCapabilities(state, { containerSide: 'A' }, 'options');
  const optionsB = getProviderCapabilities(state, { containerSide: 'B' }, 'options');
...
export const getAllowedProviderIdentities = (state, { input, meta }) => {
...
export const getUnmappedStatusesBySide = (state, containerSide) => {
  ...
  const statuses = getFieldValues(state, { kind: PCD_FIELD, fieldId: 'status', containerSide });
...
export const getSelectedProvidersMapValueFields = (state) => {
  const pcdFieldsA = getProviderCapabilities(state, { containerSide: 'A' }, 'fields');
  const pcdFieldsB = getProviderCapabilities(state, { containerSide: 'B' }, 'fields');
...
export const getUnmappedUsers = (state, { containerSide }) => {
...
export const getField = (state, {
  ...
  if (providerId) {
    ...
    capabilities = providerIdentityId
      ...
      : getProviderCapabilities(state, { containerSide });
  }
...
export const getFieldAssociationFieldValue = (state, containerSide, fieldAssociation) => {
  ...
  return getContainerFieldValue(state, { containerSide }, mappingName);
};
...
const isFieldAssociationMissingDefaults = (state, { fieldAssociation, containerSide }) => {
...
export const getFieldAssociationSideWarnings = (state, { fieldAssociationIndex, fieldAssociation, containerSide }) => {
  ...
  const unmappedCount = getUnmappedFieldValuesCount(state, { fieldAssociationIndex, containerSide, fieldAssociation });
  const isOtherSideReadOnly = isSideReadOnly(state, { containerSide: getOtherSide(containerSide) });
  ...
  const isMissingDefaultValues = isFieldAssociationMissingDefaults(state, { fieldAssociation, containerSide });
  const isReadOnly = isSideReadOnly(state, { containerSide });
...
export const getFieldAssociationWillBeIgnored = (state, {
  ...
  const isNotBothWays = getContainerFieldValue(state, { containerSide: 'A' }, 'readOnly')
    || getContainerFieldValue(state, { containerSide: 'B' }, 'readOnly') || defaultTarget !== fieldTypes.TARGET.BOTH;
  ...
  if (isNotBothWays && associationTarget && associationTarget !== fieldTypes.TARGET.BOTH) {
    ...
    return getContainerFieldValue(state, { containerSide: associationTarget }, 'readOnly') || associationTarget !== defaultTarget;
  }

  return false;
};

export const getMappingSideWarnings = (state, { containerSide }) => {
  let warnings = List();

  const fieldAssociations = getFieldAssociationsList(state);

  fieldAssociations.forEach((fieldAssociation, fieldAssociationIndex) => {
    const warning = getFieldAssociationSideWarnings(state, { fieldAssociationIndex, fieldAssociation, containerSide });
...
export const getFiltersSideWarnings = (state, { containerSide, isEdit }) => {
  const warnings = List();

  const isMultiSync = isContainerInMultiSync(state, { containerSide, isEdit });
  const isReadOnly = isSideReadOnly(state, { containerSide });
  ...
  if (isMultiSync && appliedFilters.length === 0) {
    const isOtherReadOnly = isSideReadOnly(state, { containerSide: getOtherSide(containerSide) });
    if (isOtherReadOnly) {
...
export const getUnmappedFieldValuesCount = (state, { fieldAssociation: fa, containerSide }) => {
...
export const fieldAssociationNeedsAutomapping = (state, { fieldAssociation, entity }) => {
...
export const isOrganizationOverUserLimit = (state, organizationId) => {
  ...
  const collaboratorsStats = getCollaboratorsStatsByOrgId(state, organizationId);
...
export const getOrganizationName = (state, organizationId) => fromOrganizations.getName(state.organizations, organizationId);

export const getOrganizationBillingEmail = (state, organizationId) => fromOrganizations.getBillingEmail(state.organizations, organizationId);
...
export const getOrganizationStatus = (state, organizationId) => fromOrganizations.getOrganizationStatus(state.organizations, organizationId);

export const getOrganizationTraits = (state, organizationId) => fromOrganizations.getOrganizationTraits(state.organizations, organizationId);
...
export const isOrganizationAccountSuspended = (state, organizationId) => {
  const paymentStatus = getOrganizationStatus(state, organizationId);
...
export const organizationNeedsPayment = (state, organizationId) =>
  isOrganizationAccountSuspended(state, organizationId) || isOrganizationTrialExpired(state, organizationId);
...
export const isSideReadOnly = (state, { containerSide }) => getContainerFieldValue(state, { containerSide }, 'readOnly');
...
export const isLoadedCustomFields = (state, { containerSide }) => fromFields.isLoadedCustomFields(state.fields, { containerSide });
...
export const getIsSideLocked = (state, { containerSide }) => fromApp.getIsSideLocked(state.app, containerSide);
...
export const getInitialProviderIdentityId = (state, props, providerId) => {
  const defaultProviderId = getContainerFieldValue(state, props, 'providerId') || providerId;
  const defaultProviderIdentityId = fromApp.getDefaultParamProviderIdentityId(state.app, props.containerSide);
  return defaultProviderIdentityId || getDefaultProviderIdentityByProviderId(state, defaultProviderId);
};
...
export const getInitialWorkspaceId = (state, { containerSide }) => {
  const workspaceId = fromApp.getDefaultParamWorkspaceId(state.app, containerSide);
  return workspaceId || getContainerFieldValue(state, { containerSide }, 'workspaceId');
};
...
export const getSematextExperienceToken = state => fromApp.getSematextExperienceToken(state.app);

export const getSegmentWriteKey = state => fromApp.getSegmentWriteKey(state.app);
