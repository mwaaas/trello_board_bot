...
import { billingTypes, organizationTypes } from '../consts';
...
export const isOnCustomPlan = (state, organizationId) => {
  const organization = getById(state, organizationId);
  return organization.get('planId') === organizationTypes.PLANS.CUSTOM_INVOICED;
};
...
export const getOrganizationTraits = (state, organizationId) => {
  ...
  const { members, partnerInfos, ...rest } = data;
  return rest;
};
